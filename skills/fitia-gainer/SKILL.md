...
Respect the returned `recipeProfile.activeProfile`. `legacy_v1` is the frozen reference recipe; algorithm strategy is separate. An inactive, incomplete `future_v2` is only preparation and never permission to switch profiles. Profile/ingredient changes use the same config preview, explicit approval and `expectedVersion` flow below. Top-level `adaptive` preferences apply to the historical recipe; future bounds live with the future profile.

Universe Nutrition Maltodex is an unflavored carb-source candidate, disabled by default, with undeclared protein. Never infer that unknown protein is zero, describe it as orange-flavored, or replace honey/stevia because it contains sucralose. Activation requires complete nutrition and explicit quantities/bounds; future Anchor water ratio/concentration and the Nestum limit also require explicit configuration. Do not select physical ratios or override validation yourself. Use returned ingredient amounts and scoring components; cost/practicality optimization is not enabled.
...
Adaptive defaults to `proportional_v2`: only Quaker, Anchor, Nestum and 7 Cereales vary within the returned dynamic daily bounds. Maca, cocoa, cinnamon, vanilla and water derive from their combined size. `honey_only` evaluates neighboring whole-tablespoon doses around the size-derived target; other sweeteners keep their fixed behavior. Use the selected dose returned by the calculator, not the inventory's nominal dose. Never recompute these quantities yourself or enlarge bounds to improve a score. `adaptive.strategy: legacy_v1` remains an explicit fallback using the original behavior.

Adaptive output contains the whole quantities, derived water, nutrition and cost. Its `scaleFactor` is only a calorie equivalent: **never use it to multiply the base recipe**. Explain relevant changes from `optimization.adaptive.baseline`, `bounds`, `ingredientAdjustments`, `derivedIngredients`, `honey` and the returned `reason`. Extra penalties for adding already-high protein/fat remain. This bounded search reports the best candidate found, not a guaranteed global optimum.
...
Adaptive splits contain `carryoverDraft.adaptiveAmounts`; new proportional splits also contain `adaptivePreparation` with their version, original baseline and selected honey. Preserve the entire returned draft when the user asks to save; never convert it back to a proportional scale. Old snapshots without that preparation field retain their original ingredients, honey and water. The frozen morning payload remains authoritative. Adaptive settings use the existing config preview/approval flow; nested `adaptive` patches merge, so changing one weight or ingredient deviation does not reset others.
